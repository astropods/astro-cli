...
A deployment normally moves from `deploying` to `active` within a minute or two. If it stays in `deploying` longer than expected, Astro AI flags it as "taking longer than usual" and surfaces recovery actions directly on the deployment: roll back to your last good version, pause, redeploy, or restart. These actions stay available throughout, so a stuck deploy never leaves you without a way to act.
