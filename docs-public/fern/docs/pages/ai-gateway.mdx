...
`pixtral-large` and `qwen3-vl` accept image input alongside text. Pass the image as a base64 **data URI** in an `image_url` content block — the gateway forwards the image bytes to the model, so remote URLs are not fetched server-side.
...
- **Image generation.** No image-generation models are offered. Image *input* (vision) is supported by `pixtral-large` and `qwen3-vl`.
