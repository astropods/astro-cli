...
| Env var                 | Purpose                                  |
| ----------------------- | ---------------------------------------- |
| `ASTRO_GATEWAY_URL`     | Gateway host (stable). Append `/v1` for the OpenAI-compatible API (see below). |
| `ASTRO_GATEWAY_API_KEY` | Bearer credential (stable). Treat as a secret. |
...
The five `gpt-*` models are the only ones that can [search the web](#web-search).
...
- **Anthropic server-side tools.** `web_search`, `web_fetch`, code execution, and computer use aren't available on the Claude models. Client-defined tools work normally, and the `gpt-*` models have their own [web search](#web-search).
- **Image generation.** No image-generation models are offered. Image *input* (vision) is supported by `pixtral-large`, `qwen3-vl`, and the `gpt-5-6-*` models.
- **Bring-your-own-model.** Adding new models or fine-tunes to the gateway isn't self-serve; contact support.

## Next steps

- [Deploy your first agent](/deploy-agent): ship an agent that calls the gateway
- [Monitor your agents](/monitor-agents): trace the model calls your agent makes
