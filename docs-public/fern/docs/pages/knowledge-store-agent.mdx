...
<Note>
With one store, the variables are simply `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` — the entry name (`writer`) is not part of them. That only changes when you add a second store of the same type (see [Adding a second store](#adding-a-second-store)).
</Note>
