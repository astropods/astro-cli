...
To fetch logs from the messaging workload instead:

```bash
ast agent logs --name "My Production Agent" --workload agent/messaging
```

`--workload` also targets non-agent workloads. Pass either a full workload name (`my-agent-knowledge-vectors`), an entry-name suffix (`vectors`), or a component label (`knowledge`, `collector`):
...
Append `/<container>` to pick a specific container in the resolved workload. Run `ast agent get --name "My Production Agent"` to see each workload's name.

List recent traces (the same trace data as the Monitor view in the web UI):

```bash
ast agent trace --name "My Production Agent"
```

Show the activity summary from the `/agents` dashboard (sparkline, last active, totals). Use this when you want to match what the card shows without querying the trace data directly; data updates about every 10 minutes. `--summary` ignores list window flags (`--limit`, `--start`, etc.).
...
Filter by time window or paginate (default `--limit 50`):
