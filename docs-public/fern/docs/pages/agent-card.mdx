---
title: Agent card
subtitle: Control how your agent appears in the registry with an AGENT.md file
slug: agent-card
---
...
Existing agents without an `AGENT.md` continue to work; the `meta.description` and `meta.tags` fields are still read as a fallback. See the [Agent Card Spec](/agent-card-spec) for full details on field behavior and normalization.
