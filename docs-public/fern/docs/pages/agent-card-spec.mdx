...
Entries exceeding 100 characters are truncated silently.
...
Integrations are derived from two sources: the `integrations` section of `astropods.yml` (via `provider` fields) and the `integrations` frontmatter in `AGENT.md`. The platform combines and deduplicates both sources for display. Use the agent card to declare integrations that cannot be inferred from the spec.
...
The platform maintains a registry of known integrations. Each entry contains:

| Field     | Type     | Description |
| --------- | -------- | ----------- |
| `id`      | string   | Canonical lowercase identifier used for matching. |
| `name`    | string   | Display name rendered in the UI. |
| `aliases` | string[] | Alternative strings that resolve to this integration. |
