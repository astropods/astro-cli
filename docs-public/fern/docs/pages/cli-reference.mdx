...
```bash
ast agent logs <name> [--workload <workload>[/<container>]] [--tail] [--id <deployment-id>]
```
...
Ambiguous identifiers (e.g. `knowledge` when several knowledge entries exist) return an error listing the candidates. Use `ast agent get <name>` to see each workload's K8s name.
