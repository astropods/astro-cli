...
By default, `push` builds containers, pushes images to the Astropods registry, and registers the agent spec with the server. Requires an account and [`ast login`](/authentication) first.
