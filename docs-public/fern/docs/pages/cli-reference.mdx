...
Re-deploy an existing agent in-place. Accepts the same `--adapter`, `--var`, `--vars-file`, `--build`, `--cluster`, `--dry-run`, `--wait`, and `--json` flags as `blueprint deploy`. If `--adapter` is omitted, the adapter defaults to `web`; pass `--adapter` explicitly to preserve or change a non-web setup.
