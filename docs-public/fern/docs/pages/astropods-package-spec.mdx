...
<Versions>
<Version version="v1.1 (Draft) - 2026-03-07" default>
## Abstract
...
10a. For each entry in `models`: `models` and `model` are mutually exclusive. Providing both is invalid. `model` is deprecated; implementations SHOULD emit a warning when it is used.
...
13. When `agent.interfaces.frontend` is `true`, the agent MUST serve HTTP on port 80.
14. When `agent.interfaces.messaging` is `true`, the agent MUST implement the messaging protocol.
