...
<Versions>
<Version version="v1.7 (Draft) - 2026-08-03" default>
...
| ------- | ---------- | ----------------------------------------------------------------------- |
| v1.7    | 2026-08-03 | **Breaking.** Removed `dev.overrides.playgroundImage`. It has had no effect since v1.4, when the playground UI moved into the messaging container. Remove it from `astropods.yml`; specs that still set it now fail validation. |
| v1.6    | 2026-05-12 | **Breaking.** Removed `persistent` from `knowledge.<name>` and `ContainerConfig`. Persistence is derived from the mount-path signal — the provider's mount path in provider mode, or `container.volume` in container mode. |
| v1.5    | 2026-04-24 | Deprecated `meta.visibility`. Visibility is managed via the platform UI and API, not the spec. |
| v1.4    | 2026-04-08 | Removed the standalone playground (its image and the `ast playground` command); the playground UI moved into the messaging container. |
| v1.3    | 2026-04-05 | Added `volume` field to `ContainerConfig` for custom container persistent storage. Component `inputs` now injected into knowledge, tool, and ingestion containers during `ast dev`. |
...
An Astropods Spec file (`astropods.yml`) is a YAML document that declares:
...
- A **provider** is a named, platform-known service (e.g. `anthropic`, `qdrant`). The platform resolves each provider to one of two kinds:
  - **Self-hosted**: the platform deploys and manages a container on the agent's behalf (knowledge stores such as `qdrant` or `postgres`).
  - **Cloud**: the platform injects credentials for an external API (model providers such as `anthropic` or `openai`).
...
| ----------- | ------------------------ | ------------ | ------------------------------------------------------------------ |
| `spec`      | string                   | **REQUIRED** | Spec version identifier. MUST be `blueprint/v1`.                       |
| `name`      | string                   | **REQUIRED** | Unique agent name.                                                 |
| `meta`      | object                   | **DEPRECATED** | Agent metadata. See Section 2.1.                                 |
...
| `visibility`  | string   | **DEPRECATED** | Agent visibility. Use the platform UI or API instead. |
...
| `ai_gateway`  | boolean     | OPTIONAL    | Enable the Astro AI Gateway. Injects `ASTRO_GATEWAY_URL` and `ASTRO_GATEWAY_API_KEY` into the agent container; the agent picks any supported model at call time. Default: `false`. |
...
The `interfaces` object declares how users reach the agent. Both fields are booleans indicating whether the agent has the capability. Deployment-time configuration (which messaging adapters to enable, domain routing, etc.) belongs in the deployment spec or `dev.interfaces`.

Omitting `interfaces` entirely preserves backward compatibility; the platform defaults to messaging enabled.

`interfaces` MUST be declared under the `agent` key. A common mistake is placing it at the top level of the spec, which the platform ignores.
...
When `frontend` is `true`, the agent container MUST serve HTTP on **port 80** in production (see Validation Rule 15). Use `dev.interfaces.frontend.port` to specify a different port for local development; the platform proxies that port to port 80.

```yaml
# Correct — interfaces nested under agent
agent:
  build:
    context: .
...
The `models` section declares AI models the agent consumes: LLMs (e.g. Claude, GPT), embedding models, or any model served behind an inference API. Each entry in the `models` map:

| Field       | Type            | Required    | Description                                                                             |
| ----------- | --------------- | ----------- | --------------------------------------------------------------------------------------- |
| `provider`  | string          | Conditional | Cloud model provider (e.g. `anthropic`, `openai`), `gateway` (Astro AI Gateway), or a custom provider. |
| `models`    | string[]        | OPTIONAL    | Model identifiers to make available. For `provider: gateway`, these are the selectable **options**. |
| `model`     | string          | OPTIONAL    | **Deprecated.** Use `models` instead. Single model identifier.                          |
| `container` | ContainerConfig | Conditional | Custom container configuration for a self-hosted inference server.                      |
| `inputs`    | Input[]         | OPTIONAL    | User-supplied inputs injected into the model's container.                               |

**Astro AI Gateway (`provider: gateway`).** Routes calls through the managed gateway — no provider key required. The `models` list is a menu of options; at deploy time the deployer picks one and it is injected as `MODEL_<NAME>` (e.g. `MODEL_DEFAULT`), alongside the stable `ASTRO_GATEWAY_URL` + `ASTRO_GATEWAY_API_KEY`. See [AI Gateway](/ai-gateway). This replaces the deprecated `agent.astro_ai_gateway: true` boolean.
...
A knowledge entry is **persistent** iff it has a mount path. In provider mode, the mount path comes from the provider registry — every built-in self-hosted knowledge provider (`qdrant`, `redis`, `postgres`, `mysql`, `neo4j`) declares one, so all of them are persistent by definition. In container mode, the mount path is `container.volume`; omitting it yields an ephemeral workload. Persistent entries get durable storage provisioned with platform defaults.

`inputs` applies to both provider-mode and container-mode entries. They are injected into the knowledge container at runtime — not into the agent.

The built-in `postgres` provider manages its own credentials automatically: the platform generates a random password and a platform-managed user, then injects `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB` into the agent container via secrets. No `inputs` are needed on the knowledge entry:
...
In your agent, always read all five env vars — never hardcode user or password:
...
For custom containers that need configuration, use `inputs` to pass values into the container:

> **`secret: true` and `ast project start`**: Inputs marked `secret: true` are stored securely and never logged. However, because they have no plaintext default, they require `ast project configure` to be run before `ast project start` can start the container. If you need the container to start immediately in local dev without a manual configure step, use a non-secret input with a `default` value instead.
...
| `volume`      | string                | OPTIONAL    | Mount path for persistent data inside the container. Setting `volume` makes the entry persistent (durable storage is provisioned); omitting it yields an ephemeral workload. Has no effect when `provider` is set — provider-mode entries inherit the provider's mount path. |
| `environment` | map\<string, string\> | OPTIONAL    | Static environment variables passed directly to the container at runtime. Use for non-sensitive configuration (database names, feature flags, log levels). For secrets, prefer `inputs` with `secret: true`. |
...
#### Configuring Containers with Inputs

Use `inputs` to declare values that the container needs at runtime. Each input is prompted via `ast configure` at deploy time and injected as an environment variable into the container. Values marked `secret: true` are stored securely and never logged.
...
The `dev` section provides local development overrides consumed by `ast project start`. These fields are deployment concerns that do not belong in the normative agent topology.
...
When the entry name equals the provider name, the redundant qualified form (e.g. `ANTHROPIC_ANTHROPIC_API_KEY`) is omitted and only the bare key is produced.
...
Example: `inputs: [{name: OPENAI_API_KEY, datatype: string, secret: true}]` → `OPENAI_API_KEY` in the target container.
...
All built-in model providers are cloud (credentials-only). To self-host an inference server, use a `container` definition instead of a `provider`.
