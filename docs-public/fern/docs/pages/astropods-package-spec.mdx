...
<Versions>
<Version version="v1.0 (Draft) - 2026-02-27" default>
## Abstract
...
| `visibility`  | string   | OPTIONAL | Agent visibility. MUST be one of `public` or `private`. When omitted, the CLI prompts on first push; defaults to `private` in non-interactive environments. Public agents appear in the catalog and are accessible to anyone. Private agents are only visible to account members. |
