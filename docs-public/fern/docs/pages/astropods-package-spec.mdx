...
| ------- | ---------- | ----------------------------------------------------------------------- |
| v1.3    | 2026-04-05 | Added `volume` field to `ContainerConfig` for custom container persistent storage. `environment` on knowledge containers now passed through to Docker. Clarified `environment` vs `inputs` usage. |
...
#### `environment` vs `inputs`

`environment` and `inputs` both inject environment variables, but serve different purposes:

- **`environment`** is declared on the container and passed directly to that container at runtime. Values are static and visible in the deployment configuration. Best for non-sensitive settings the container needs to start: database names, ports, feature flags, log levels.
- **`inputs`** are declared on the component (knowledge, model, integration, or ingestion entry) and prompted via `ast configure` at deploy time. Values marked `secret: true` are stored securely and never logged. Best for credentials, API keys, and passwords.
...
knowledge:
  db:
    container:
      ...
      port: 5432
      environment:
        POSTGRES_DB: my_database         # non-sensitive, static config
    inputs:
      - name: POSTGRES_PASSWORD          # sensitive, prompted at deploy time
        datatype: string
        secret: true
