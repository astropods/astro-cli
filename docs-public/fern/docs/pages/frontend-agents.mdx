...
```mermaid
flowchart LR
    A[User] --> B[Built-in chat UI<br/>chat / Slack / OIDC] --> C[Agent container]
```
...
```mermaid
flowchart LR
    D[User] --> E[OIDC sign-in<br/>HTTPS :443] --> F[Agent container :80]
```
