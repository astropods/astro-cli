...
If your database is behind a firewall but reachable over the public network, allowlist Astro's outbound IPs first — see [Allowlist Astro's egress IPs](/ip-allowlisting).
...
<Steps>
  ...
  <Step title="Approve the connection in AWS">
    The store's detail page links you to the right AWS console page. In **VPC → Endpoints**, approve the pending connection request from Astro.
  </Step>
  <Step title="Wait for Ready">
    Astro completes the connection and verifies reachability automatically. The store turns **Ready** when the path is live — no further action needed.
  </Step>
</Steps>
...
- [Allowlist Astro's egress IPs](/ip-allowlisting): reach a firewalled database over the public network
