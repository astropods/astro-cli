...
Cost is calculated from the token counts Claude Code reports, so a very recently
released model can show usage and token counts with no cost attached until
pricing for it is available. Request counts and per-request figures are reported
alongside token usage.

<Note>
Claude Code spend history starts fresh from the release that introduced this
reporting. Usage recorded before then is not carried into your dashboard.
</Note>
