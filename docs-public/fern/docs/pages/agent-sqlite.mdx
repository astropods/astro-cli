...
[SQLite](https://www.sqlite.org/) is the easiest way to use that space. It's a small database that lives in a single file, with no separate server to run. Save the file under `/data` and your agent can remember things: past conversations, user preferences, notes, counters — whatever you need.
...
Open a database file inside `/data` and use it like any other database. The folder is already there and ready to write to. The examples below use Python and Node, but the same idea works in any language with a SQLite library — just point the file at `/data`.
...
If you need several copies of your agent to share the same data, use a [knowledge store](/docs/pages/knowledge-stores) (like Postgres or Redis) instead of a SQLite file. Otherwise, keep your agent at one copy.
