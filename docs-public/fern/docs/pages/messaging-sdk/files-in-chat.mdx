...
Astro's web chat can send files to an agent and render files from the agent as download chips. The transfer has two parts:
...
The browser never puts file bytes in a chat message. The agent normally does not call the Files API either: Astro mounts the same file store into the agent container and sets `AGENT_FILES_DIR` to its location (normally `/data/files`).
...
Declare a messaging-capable agent. The deployed web chat and `ast dev` provision and mount the file store automatically; there is no separate files setting.
...
When you use an Astro adapter, each call to your agent receives the current turn's files in `StreamOptions.attachments`. Treat this list as the authority for the turn. Do not enumerate the whole shared directory to decide which files belong to the current user.
...
const adapter: AgentAdapter = {
  ...
  getConfig() {
    return { systemPrompt: "Analyze user-provided documents.", tools: [] };
  },
};
```
...
See the [Node messaging SDK](./node#inbound-message-anatomy) or [Python messaging SDK](./python#inbound-message-anatomy) for the full message and response shapes.

## Build a custom chat client

Astro's built-in chat already implements this flow. A custom client must use both APIs in order:
...
- The agent container shares the underlying volume, so agent code must use only the current turn's attachment paths for input. Do not scan or expose unrelated files.
- Treat attachment bytes, display names, and MIME types as untrusted input. Apply the same parser limits and content validation you would use for any user upload.
