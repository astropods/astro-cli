...
**Daily cap.** The per-episode dedup still lets a *flapping* deployment (resolve → re-breach repeatedly) emit once per episode. A second choke point caps observation alerts to **one send per (deployment, condition) per rolling 24h**. A persistent ledger `deployment_alert_notifications` (deployment_id, condition, last_notified_at) records each send and — unlike `deployment_alert_state` — is **not** cleared on resolve, so it spans episodes. Before emitting a firing edge the evaluator does an atomic claim (upsert with a conditional update on `last_notified_at < now − 24h`); it sends only when it wins the claim, otherwise it suppresses and logs. Firing state is still marked so the episode isn't re-evaluated. Distinct conditions are capped independently (crash loop and OOM on one agent can both alert the same day); the cap is per (deployment, condition), not per severity or per account.

**Three workflows by severity.** Conditions do **not** map 1:1 to Novu workflows. Every condition carries a `Severity` (`critical`, `warning`, or `info`) and collapses to one of three workflows: `observation.critical` ("Agent failing" — crash loop, OOM, unschedulable), `observation.warning` ("Agent degraded" — restarts, memory/compute pressure, error spikes), or `observation.info` ("Agent over-provisioned" — CPU/memory usage far below request, a cost/waste signal rather than a health problem; both of its conditions are currently disabled, so the workflow has no producer). The specific condition rides in the payload `reason` (short title, e.g. "Out of memory") plus `details` (a one-line explanation of what happened, e.g. "A container was killed for exceeding its memory limit"), so three shared templates render every condition. This gives the user **three preference toggles**, not one per condition, and adding a condition needs no new workflow. Firing state stays keyed on the granular condition name so two same-severity conditions on one deployment don't collide.
...
| `cpu_over_provisioned` *(disabled)* | CPU usage far below its request, sustained (waste) | info → `observation.info` |
| `memory_over_provisioned` *(disabled)* | memory usage far below its request, sustained (waste) | info → `observation.info` |
...
**Throttle / digest.** Observation conditions are rate-limited by their firing-state machine (edge-only, no re-emit while firing) and a daily cap of one send per (deployment, condition) per rolling 24h (§Observation alerts → Daily cap), which bounds a flapping deployment. Where a single deployment can trip several conditions at once, a Novu digest step (or worker-side throttle keyed on `subscriber + deployment + window`) rolls them into one notification. Window per-type (e.g. 15 min).
