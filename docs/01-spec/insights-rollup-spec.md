...
- **The v2 metrics API upgrade path is now known — see `docs/06-plan/langfuse-v4-migration.md`.** v2 removes the `traces` view entirely and marks `userId` high-cardinality, both confirmed against a live v4 instance. `view: "traces"` is replaced with `view: "observations"` filtered to `isRootObservation=true` for count-family measures (cost/token measures must stay unfiltered, since they live on child `GENERATION` spans and don't roll up onto the root). `userId` grouping requires `orderBy` + `config.row_limit`, capped at 1000 server-side with no larger value available — a permanent ceiling for any account with more than 1000 active users in a day, mitigated by ordering results by the query's leading measure so a capped account still gets its top spenders rather than an arbitrary slice. `client.go`'s `v4Reader` implements all of this; `fetchUsageGrain` already calls it correctly. This is no longer a reason to prioritize the phase-5 ingest producer over the current ETL producers — both now work against v4.
