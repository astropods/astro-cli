...
- **KMS envelope encryption pattern.** `internal/langfuse/provisioner.go` already encrypts the Langfuse secret key with the deployment KMS key and stores ciphertext in Postgres; that pattern is the direct template for storing the LiteLLM virtual key.
