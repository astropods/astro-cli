...
| `billing.dunning_suspended` † | Dunning sweep suspends account (`riverqueue/billing_dunning.go`), or the card-expiry sweep does (`riverqueue/billing_card_expiry.go`) | managers | `{"account": "acme", "ctaUrl": "/settings/billing"}` (`account` is best-effort and renders `""` if the account lookup fails) | **Account suspended** — **{account}** has been suspended for non-payment. Update your payment method to restore service. → Manage billing |
