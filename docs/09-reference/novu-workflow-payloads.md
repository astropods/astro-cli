...
| `security.key_changed` (`revoked`) † | OTel ingest token revoked (`handlers/otel_ingest_tokens.go`) | managers | `{"keyKind": "OTel ingest", "keyName": "prod-collector", "action": "revoked", "ctaUrl": "/settings/api-keys"}` | **{keyKind} key revoked** — The {keyKind} key "**{keyName}**" was revoked. → Manage API keys |
