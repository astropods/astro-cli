...
Add the minimal WorkOS FGA client needed by downstream deployment lifecycle and enforcement work. The client consumes the `deployment:read` and `deployment:edit` permission contracts defined by its PR2 base without changing authorization behavior.
...
No user or deployment migration is required. Configure the deployment resource type, permissions, and reader/editor roles in the preview WorkOS environment before PR4 introduces live resource writes.
