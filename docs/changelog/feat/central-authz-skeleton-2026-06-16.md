...
Introduce a centralized authorization seam in astro-server ahead of WorkOS FGA deployment-level checks. No runtime behavior change — routes and handlers are untouched.

This is the first step in a multi-step effort to restrict per-deployment mutations to org owner/admin or a deployment admin via WorkOS FGA.
