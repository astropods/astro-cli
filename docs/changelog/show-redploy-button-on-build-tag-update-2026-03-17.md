...
- **Post-deploy cache strategy**: for redeploys the mutation optimistically patches the deployment's `build_id` in the TanStack Query cache via `setQueryData`, so badges clear immediately without waiting for the server to reconcile. For fresh installs (no existing cache entry) the deployments query is invalidated instead so the new deployment appears on the list after redirect.
- **Polling efficiency**: `refetchIntervalInBackground` is explicitly `false` so polling pauses when the tab is not focused.

E2E coverage is split across two suites:

- **`build-update-badge.spec.ts`** — badge on agents list card, badge on detail header, negative case for up-to-date deployments, current-build-id badge, build-only redeploy submission, badge clearing after successful redeploy (optimistic cache), action bar transition from build-only to dirty "Save & Redeploy" and back via Cancel, and failed-redeploy error resilience.
- **`fresh-install.spec.ts`** — fresh deploy adds a new deployment to the agents list after redirect (catches invalidation regression), re-deploying an existing agent keeps its card visible (catches optimistic-patch corruption).

The mock backend is stateful — deploy responses update the in-memory deployment list (adding new entries for fresh installs, patching `build_id` for redeploys), and a `/test/reset` endpoint restores initial state between tests. Playwright is configured with `workers: 1` so all tests run sequentially against the shared mock.
