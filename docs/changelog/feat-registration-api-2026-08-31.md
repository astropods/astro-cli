...
validate endpoint checks a document without persisting anything. Dataset
item admission and the review queue were also relaxed to key off a trace's
own evaluation ref instead of the agent's current one, so older results
stay visible after the active set changes.
