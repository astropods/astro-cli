...
- `cd apps/astro-client && bun x playwright test e2e/org-permissions.spec.ts e2e/org-members.spec.ts`: role-gated org nav and members against the mock backend. These run in CI as `astro-client:e2e` and assert on nav labels by text, so a rename here fails there even when vitest is green.
