...
-- agent_name is a denormalized snapshot for portability; deployment_id is nullable so
-- history survives deployment removal (ON DELETE SET NULL) for future account export.
CREATE TABLE public.deployment_chat_conversations (
    ...
    CONSTRAINT deployment_chat_conversations_deployment_id_fkey FOREIGN KEY (deployment_id) REFERENCES public.deployments(id) ON DELETE SET NULL,
...
CREATE INDEX idx_deployment_chat_conversations_account_user ON public.deployment_chat_conversations(account_id, user_id, updated_at DESC);
