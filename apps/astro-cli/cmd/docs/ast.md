...
This creates a starter project with all required assets (spec, Dockerfile, agent code, and config). Once `ast dev` is running, your agent is available locally; add interfaces (e.g. Slack) in the create wizard or by editing `astropods.yml` and `.env`.
...
The CLI reads `astropods.yml` to generate Docker Compose for `ast dev`, to build images for `ast build`, and to push images and the spec for `ast push`. Edit this file to add/remove integrations, interfaces, or knowledge, and ensure `.env` has the matching keys.

## Commands

### `create` — Scaffold a starter project

Create a new agent project with the required structure, `astropods.yml`, Dockerfile, agent code, and `.env`. Run this first, then `cd` into the directory and run `ast dev`.
...
`ast dev` loads `.env`, generates a Docker Compose project from `astropods.yml`, builds and starts all containers, then exits. Containers keep running in the background.
...
**Against a deployed agent:** use the dedicated hostname that the Astro platform gives the agent after deployment, e.g. `ast playground https://your-agent.agent.astropods.ai`.
...
- `--file, -f` — Path to spec file (default: `astropods.yml`)
