...
// Parity with the web UI's Agents → <name> → Monitor flow:
//   - no --trace-id: list recent traces (the Monitor tab)
//   - with --trace-id: show that trace's Overview (input, output, metadata, observations)

var agentTraceCmd = &cobra.Command{
	Use:   "trace",
	Short: "List traces for a deployed agent, or show a single trace",
