#!/usr/bin/env node
// Fails if local-theme/no-static-inline-style violations exceed BASELINE.
...
const count = results.reduce(
  (n, f) => n + f.messages.filter((m) => m.ruleId === "local-theme/no-static-inline-style").length,
  0,
);

if (count > BASELINE) {
  console.error(
    `local-theme/no-static-inline-style: ${count} violations, baseline is ${BASELINE}. ` +
...
if (count < BASELINE) {
  console.log(
    `local-theme/no-static-inline-style: ${count} violations, baseline is still ${BASELINE}. ` +
  ...
  console.log(`local-theme/no-static-inline-style: ${count} violations (baseline ${BASELINE}).`);
}
