...
test("shows min-length hint and disables button when name is too short", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: /create blueprint/i })).toBeDisabled();
});

test("shows letter-start hint and disables button when name begins with a digit", async ({ page }) => {
  test.setTimeout(20_000);
  await page.goto("/new/custom", { waitUntil: "domcontentloaded" });
  await expect(page.getByRole("button", { name: /create blueprint/i })).toBeVisible({ timeout: 10_000 });
...
test("shows 'already exists' error and disables button for an existing blueprint name", async ({ page }) => {
  test.setTimeout(20_000);
  await page.goto("/new/custom", { waitUntil: "domcontentloaded" });
  await expect(page.getByRole("button", { name: /create blueprint/i })).toBeVisible({ timeout: 10_000 });
...
test("shows 'will be created as' hint and enables button for a valid available name", async ({ page }) => {
  test.setTimeout(20_000);
  await page.goto("/new/custom", { waitUntil: "domcontentloaded" });
  await expect(page.getByRole("button", { name: /create blueprint/i })).toBeVisible({ timeout: 10_000 });
  ...
  await expect(page.getByRole("button", { name: /create blueprint/i })).toBeEnabled();
});
