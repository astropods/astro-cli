...
To develop the frontend against a deployed backend (e.g., `https://example.com`), you need to set up local HTTPS with a same-site domain. This is required for authentication cookies to work properly.
...
1. Add `local.example.com` to your `/etc/hosts` (requires sudo)
...
Open `https://local.example.com:5173` in your browser.
...
1. Your local frontend runs at `https://local.example.com:5173`
2. The backend runs at `https://example.com`
3. Both share the same registrable domain (`example.com`)
...
- You're accessing the app via `https://local.example.com:5173` (not `localhost`)
...
Ensure:
1. The backend has `http://localhost:5173` and `https://local.example.com:5173` in `ALLOWED_ORIGINS`
2. The backend has `AUTH_COOKIE_DOMAIN=.example.com` set
