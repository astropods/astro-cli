...
export const meta: MetaFunction = () => [
  { title: "OAuth apps - Settings | Astro" },
];

const COLUMN_COUNT = 4;

export function AppsPanel({ account: accountName }: { account: string }) {
  ...
  const apps = useApps(accountName);
  const appScopes = useAppScopes(accountName, creating);
  const createApp = useCreateApp(accountName);
...
export default function AppsSettings() {
  ...
  return <AppsPanel account={personalAccount?.name ?? ""} />;
}
