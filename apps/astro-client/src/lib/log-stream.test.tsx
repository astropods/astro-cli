...
import { LogStreamProvider, useLogStream } from "./log-stream";
...
describe("LogStreamProvider", () => {
  ...
  it("event: ready sets status to tailing", () => {
    const { result } = renderHook(() => useLogStream(), { wrapper });
  ...
  it("closes the EventSource when the provider unmounts mid-stream", () => {
    ...
    // If unmount cleanup regresses, an open EventSource leaks for every
    // navigation away from the logs tab — eventually exhausts the browser's
    // per-origin SSE connection limit and breaks the page.
    expect(es.closed).toBe(true);
  });
