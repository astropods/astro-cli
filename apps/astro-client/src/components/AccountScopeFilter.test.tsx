...
const auth = {
  ...
  accounts: [
    ...
    { id: "org", name: "acme", display_name: "Acme", type: "org" },
  ],
};
...
describe("AccountScopeFilter", () => {
  it("switches the selected account without changing global session scope", async () => {
    const user = userEvent.setup();
    renderRoute([{ path: "/", Component: Harness }], { auth });

    const trigger = screen.getByRole("combobox", {
      name: "Scope by account",
    });
    ...
    const personalOption = screen.getByRole("option", { name: /Test User/ });
    expect(personalOption).toHaveAttribute("data-state", "checked");
    expect(personalOption.querySelector(".lucide-check")).not.toBeNull();
    ...
    await user.click(trigger);
    const organizationOption = screen.getByRole("option", { name: /Acme/ });
    expect(organizationOption).toHaveAttribute("data-state", "checked");
    expect(organizationOption.querySelector(".lucide-check")).not.toBeNull();
  });
});
