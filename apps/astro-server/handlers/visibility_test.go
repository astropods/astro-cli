...
func TestArchiveAgent_Success(t *testing.T) {
	...
	router.POST("/agents/:account/:name/archive", injectTestAccount(), ArchiveAgent(log, index, nil, nil, nil, nil, nil, nil))
...
func TestArchiveAgent_NoAccount(t *testing.T) {
	...
	router := gin.New()
	// No account injected
	router.POST("/agents/:account/:name/archive", ArchiveAgent(log, index, nil, nil, nil, nil, nil, nil))
...
func TestGetAgent_PublicAgent_NoAuth(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT .+ FROM agents WHERE account_id").
		...
		WillReturnRows(sqlmock.NewRows([]string{"account_id", "name", "registry", "visibility", "archived_at", "name_reserved", "avatar_colors", "avatar_updated_at", "created_at", "updated_at"}).
...
func TestGetAgent_PrivateAgent_NoAuth(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT .+ FROM agents WHERE account_id").
		WithArgs("acct-1", "my-agent").
		WillReturnRows(sqlmock.NewRows([]string{"account_id", "name", "registry", "visibility", "archived_at", "name_reserved", "avatar_colors", "avatar_updated_at", "created_at", "updated_at"}).
			AddRow("acct-1", "my-agent", "registry.io", "private", nil, false, nil, nil, now, now))
...
func TestListAgents_OnlyPublic(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT .+ FROM agents a.+WHERE a.visibility = 'public'").
		WillReturnRows(sqlmock.NewRows([]string{
			...
			AddRow("acct-1", "public-agent", "registry.io", "public", nil, nil, now, now,
