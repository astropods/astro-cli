...
func TestGetAgent_PublicAgent_NoAuth(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT .+ FROM agents WHERE account_id").
		...
		WillReturnRows(sqlmock.NewRows([]string{"account_id", "name", "registry", "visibility", "archived_at", "name_reserved", "created_at", "updated_at"}).
...
func TestGetAgent_PrivateAgent_NoAuth(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT .+ FROM agents WHERE account_id").
		WithArgs("acct-1", "my-agent").
		WillReturnRows(sqlmock.NewRows([]string{"account_id", "name", "registry", "visibility", "archived_at", "name_reserved", "created_at", "updated_at"}).
			AddRow("acct-1", "my-agent", "registry.io", "private", nil, false, now, now))
