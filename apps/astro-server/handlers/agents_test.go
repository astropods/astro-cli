...
func TestRegisterAgent_Success(t *testing.T) {
	...
	router.POST("/api/v1/agents/:account/:name/register", injectTestAccount(), RegisterAgent(log, index, "", nil, nil, nil, nil, nil, false, AuthorizationResourceLifecycleDeps{}))
...
func TestRegisterAgent_RejectsOrgScopedName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			log := logger.New("error", "json")
			router.POST("/api/v1/agents/:account/:name/register", injectTestAccount(), RegisterAgent(log, nil, "", nil, nil, nil, nil, nil, false, AuthorizationResourceLifecycleDeps{}))
...
func TestCreateBlueprint_Success(t *testing.T) {
	...
	router.POST("/api/v1/agents/:account", injectTestAccount(), CreateBlueprint(log, index, nil, nil, nil, nil, AuthorizationResourceLifecycleDeps{}))
...
func TestRegisterAgent_VersionGate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router.POST("/api/v1/agents/:account/:name/register", injectTestAccount(), RegisterAgent(log, index, tt.minVersion, nil, nil, nil, nil, nil, false, AuthorizationResourceLifecycleDeps{}))
...
func TestListAccountAgents_QueryFilterPassedToIndex(t *testing.T) {
	...
	accountMock.ExpectQuery("SELECT a.id, a.name, a.type").
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "type", "workos_org_id", "deleted_at", "created_at", "updated_at", "display_name", "avatar_colors", "avatar_updated_at", "account_number", "bio", "location", "local_timezone", "pronouns", "website", "social_links", "blueprint_order"}).
...
func TestListAccountAgents_Member_PrivateVisibilityFilter(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT a.account_id, a.name, a.registry").
		...
		WillReturnRows(sqlmock.NewRows(blueprintAccountListColumns(true)).
			AddRow("test-account-id", "secret-agent", "registry.example.com", "private", nil, nil, now, now,
...
func TestListAccountAgents_Member_NoVisibilityFilter(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT a.account_id, a.name, a.registry").
		...
		WillReturnRows(sqlmock.NewRows(blueprintAccountListColumns(true)).
			AddRow("test-account-id", "public-agent", "registry.example.com", "public", nil, nil, now, now,
				"build-1", "ns", `{"name":"test"}`, "", "", "[]", now, now, "", "", "", 1, 2).
			AddRow("test-account-id", "private-agent", "registry.example.com", "private", nil, nil, now, now,
...
func TestListAccountAgents_PublishersPopulated(t *testing.T) {
	...
	indexMock.ExpectQuery("SELECT a.account_id, a.name, a.registry").
		...
		WillReturnRows(sqlmock.NewRows(blueprintAccountListColumns(true)).
			AddRow("test-account-id", "test-agent", "registry.example.com", "public", nil, nil, now, now,
				"build-1", "ns", `{"name":"test"}`, "", "", "[]", now, now, "", "", "", 1, 1))
	...
	accountMock.ExpectQuery("SELECT.*WHERE am.user_id").
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "type", "workos_org_id", "created_at", "updated_at", "display_name", "avatar_updated_at"}).
			AddRow("user-account-id", "janesmith", "personal", "", now, now, "", nil))
