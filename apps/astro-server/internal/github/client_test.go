...
func TestClient_GetDirs(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := map[string]any{
			"tree": []map[string]any{
				...
				{"path": "svc", "type": "tree"},
				{"path": "svc/handler.go", "type": "blob"},
				{"path": "svc/sub", "type": "tree"},
			},
		}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(resp)
	...
	c := newTestClient(t, srv)
	dirs, err := c.GetDirs(context.Background(), "owner/repo", "main")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := []string{"src", "svc", "svc/sub"}
...
func TestClient_GetDirs_HTTPError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "not found", http.StatusNotFound)
	}))
	defer srv.Close()
	...
	c := newTestClient(t, srv)
	_, err := c.GetDirs(context.Background(), "owner/repo", "main")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
}

func TestClient_GetDirs_EmptyRepo(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := map[string]any{"tree": []map[string]any{}}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(resp)
	}))
	defer srv.Close()
