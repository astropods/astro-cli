...
var accountColumns = []string{"id", "name", "type", "workos_org_id", "deleted_at", "created_at", "updated_at", "display_name", "avatar_colors"}
...
func TestWork_ParsesAgentLabel(t *testing.T) {
	...
	mock.ExpectQuery("SELECT a.id").
		...
		WillReturnRows(sqlmock.NewRows(accountColumns).
			AddRow("uuid-acct-1", "acct-1", "personal", nil, nil, time.Now(), time.Now(), "", nil))
...
func TestWork_SkipsMalformedAgentLabels(t *testing.T) {
	...
	mock.ExpectQuery("SELECT a.id").
		WithArgs("good-acct").
		WillReturnRows(sqlmock.NewRows(accountColumns).
			AddRow("uuid-good-acct", "good-acct", "personal", nil, nil, time.Now(), time.Now(), "", nil))
