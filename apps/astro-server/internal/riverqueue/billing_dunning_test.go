...
func (f *fakeDunningQueue) InsertBillingResume(_ context.Context, accountID string) error {
	f.resumed = append(f.resumed, accountID)
	return nil
}
...
// The scenario the read override alone does not fix: the account was suspended
// and its deployments stopped, then it became exempt. Every read now says
// active, so nothing looks wrong, while the workloads stay stopped. The sweep
// only scans past_due rows, so this is the one thing that brings them back.
func TestDunningSweep_ResumesAnExemptAccountThatWasAlreadySuspended(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock: %v", err)
	}
	...
	// No past_due rows, so the dunning loop does nothing and only the exempt
	// reconcile can produce a resume.
	mock.ExpectQuery(`FROM account_billing_status\s+WHERE \(status`).
		WillReturnRows(sqlmock.NewRows([]string{"account_id"}))
	mock.ExpectBegin()
	mock.ExpectQuery("FOR UPDATE").WithArgs("acct_exempt").
		WillReturnRows(dunningRow(billing.StatusSuspended, billing.ReasonCreditsExhausted, nil))
	mock.ExpectExec("account_billing_status").WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	q := &fakeDunningQueue{}
	w := &DunningSweepWorker{
		status:   billing.NewStatusStore(db, 7).WithExemptAccounts([]string{"acct_exempt"}),
		queue:    q,
		accounts: fakeAccountNamer{},
		log:      logger.New("error", "json"),
	}
	if err := w.Work(context.Background(), &river.Job[DunningSweepArgs]{}); err != nil {
		t.Fatalf("Work: %v", err)
	}
	if len(q.resumed) != 1 || q.resumed[0] != "acct_exempt" {
		t.Errorf("resumed = %v, want [acct_exempt]: the deployments stay stopped", q.resumed)
	}
	if len(q.suspended) != 0 {
		t.Errorf("suspended = %v, want none for an exempt account", q.suspended)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
...
// Without an exemption configured the sweep must not resume anything, or the
// case above would pass on a worker that resumes unconditionally.
func TestDunningSweep_NoExemptionsResumesNothing(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock: %v", err)
	}
