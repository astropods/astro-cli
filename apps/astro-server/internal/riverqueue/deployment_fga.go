	...
	case authz.DeploymentFGARegistered:
		...
		if firstRegistration || work.CreatorAssignmentPending {
			...
			case work.MembershipID != "":
				if err := w.fga.AssignRole(ctx, authz.MembershipAssignmentSubject(work.MembershipID), authz.RoleDeploymentEditor, resource); err != nil && !errors.Is(err, authz.ErrRoleAssignmentExists) {
					return w.fail(ctx, work, err)
				}
