...
func (m *BillingStateManager) emitActiveBilling(ctx context.Context, now time.Time) {
	rows, err := m.db.QueryContext(ctx, `
		SELECT bs.deployment_id, bs.component, bs.last_emitted_at,
		       ...
		       d.account_id, d.agent_name, d.namespace
	...
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query active billing states", "error", err)
		return
	}
	...
	for rows.Next() {
		...
		var replicas int
		if err := rows.Scan(&deploymentID, &component, &lastEmitted, &cpuReq, &memReq, &replicas, &accountID, &agentName, &namespace); err != nil {
			m.log.Error("openmeter: failed to scan active billing row", "error", err)
			continue
		}
		...
		events = append(events, computeUsageEvent(accountID, agentName, namespace, component, cpuReq, memReq, replicas, cu*elapsed))
	}
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate active billing rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: reconciled active compute", "events", len(events))
	}
	...
	if _, err := m.db.ExecContext(ctx, query, params...); err != nil {
		m.log.Error("openmeter: failed to advance billing timestamps", "error", err)
	}
}
...
func (m *BillingStateManager) reconcileStale(ctx context.Context) {
	rows, err := m.db.QueryContext(ctx, `
		SELECT bs.deployment_id, bs.component, bs.last_emitted_at,
		       bs.cpu_request, bs.memory_request, bs.replicas,
		       d.account_id, d.agent_name, d.namespace, d.status_changed_at
		FROM deployment_billing_state bs
		JOIN deployments d ON d.id = bs.deployment_id
		WHERE bs.billing_active = true AND d.status NOT IN ('active', 'provisioning')
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query stale billing states", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&deploymentID, &component, &lastEmitted, &cpuReq, &memReq, &replicas,
			&accountID, &agentName, &namespace, &statusChangedAt); err != nil {
			m.log.Error("openmeter: failed to scan stale billing row", "error", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate stale billing rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: reconciled stale compute", "events", len(events))
	}
	...
	`); err != nil {
		m.log.Error("openmeter: failed to deactivate stale billing rows", "error", err)
	}
}
...
func (m *BillingStateManager) reconcileStopped(ctx context.Context) {
	...
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query stopped billing rows", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&deploymentID, &component, &lastEmitted, &stoppedAt,
			&cpuReq, &memReq, &replicas, &accountID, &agentName, &namespace); err != nil {
			m.log.Error("openmeter: failed to scan stopped billing row", "error", err)
			continue
		}
		keys = append(keys, rowKey{deploymentID, component})
	...
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate stopped billing rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: emitted final period for stopped deployments", "events", len(events))
	}
	...
	if _, err := m.db.ExecContext(ctx, query, params...); err != nil {
		m.log.Error("openmeter: failed to clear stopped_at on billing rows", "error", err)
	}
}
...
func (m *BillingStateManager) healMissingBillingRows(ctx context.Context, now time.Time) {
	...
	`, now)
	if err != nil {
		m.log.Error("openmeter: failed to heal missing billing rows", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&deploymentID, &component); err != nil {
			m.log.Error("openmeter: failed to scan healed billing row", "error", err)
			continue
		}
		m.log.Info("openmeter: healed missing billing row", "deployment_id", deploymentID, "component", component)
	}
}
...
func (m *BillingStateManager) emitActiveKnowledgeBilling(ctx context.Context, now time.Time) {
	...
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query active knowledge billing", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storeID, &lastEmitted, &provider, &accountID, &name); err != nil {
			m.log.Error("openmeter: failed to scan knowledge billing row", "error", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate knowledge billing rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: reconciled active knowledge compute", "events", len(events))
	}
	...
	if _, err := m.db.ExecContext(ctx, query, params...); err != nil {
		m.log.Error("openmeter: failed to advance knowledge billing timestamps", "error", err)
	}
}
...
func (m *BillingStateManager) healMissingKnowledgeBillingRows(ctx context.Context, now time.Time) {
	...
	`, now)
	if err != nil {
		m.log.Error("openmeter: failed to heal missing knowledge billing rows", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storeID); err != nil {
			m.log.Error("openmeter: failed to scan healed knowledge billing row", "error", err)
			continue
		}
		m.log.Info("openmeter: healed missing knowledge billing row", "store_id", storeID)
	}
}
...
func (m *BillingStateManager) reconcileStaleKnowledge(ctx context.Context) {
	...
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query stale knowledge billing", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storeID, &lastEmitted, &provider, &accountID, &name, &updatedAt); err != nil {
			m.log.Error("openmeter: failed to scan stale knowledge row", "error", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate stale knowledge rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: reconciled stale knowledge compute", "events", len(events))
	}
	...
	`); err != nil {
		m.log.Error("openmeter: failed to deactivate stale knowledge billing rows", "error", err)
	}
}
...
func (m *BillingStateManager) reconcileStoppedKnowledge(ctx context.Context) {
	...
	`)
	if err != nil {
		m.log.Error("openmeter: failed to query stopped knowledge billing rows", "error", err)
		return
	}
	defer rows.Close() //nolint:errcheck
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storeID, &lastEmitted, &stoppedAt, &provider, &accountID, &name); err != nil {
			m.log.Error("openmeter: failed to scan stopped knowledge billing row", "error", err)
			continue
		}
		ids = append(ids, storeID)
		elapsed := stoppedAt.Sub(lastEmitted).Hours()
	...
	if err := rows.Err(); err != nil {
		m.log.Error("openmeter: failed to iterate stopped knowledge billing rows", "error", err)
	}
	...
	if len(events) > 0 {
		...
		m.log.Info("openmeter: emitted final period for stopped knowledge stores", "events", len(events))
	}
	...
	if _, err := m.db.ExecContext(ctx, query, ids...); err != nil {
		m.log.Error("openmeter: failed to delete stopped knowledge billing rows", "error", err)
	}
}
