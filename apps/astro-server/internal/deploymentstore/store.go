...
func (s *Store) SaveDeployment(id, accountID, agentName, displayName, buildID, namespace, specJSON string) (*Deployment, error) {
	...
	// If display_name is empty, fall back to matching by agent_name only when there's
	// exactly one active deployment (backward compat).
	if displayName != "" {
		_, err = tx.Exec(`
			UPDATE deployments
	...
	var d Deployment
	err = tx.QueryRow(`
		INSERT INTO deployments (id, account_id, agent_name, build_id, namespace, display_name, deployment_spec_json, status, deployed_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, 'active', NOW())
		RETURNING id, account_id, agent_name, build_id, namespace, display_name, deployment_spec_json, status, deployed_at
	`, id, accountID, agentName, buildID, namespace, displayName, specJSON).Scan(
