...
// ---------------------------------------------------------------------------
// Resolved keys consistency tests
//
// The drift detector compares ConfigMap/Secret keys stored in
// deployment_resolved_keys (written by SaveNormalizedSpec at deploy time)
// against the live K8s ConfigMap/Secret. If the normalizer stores different
// keys than the spec applier creates, every deployment shows env/secret drift.
// ---------------------------------------------------------------------------

// TestResolvedKeys_MatchesSpecApplier verifies that the resolved keys stored in
// the DB after a deploy match what the spec applier would create in K8s.
//
// The spec applier (k8s/spec_applier.go) calls:
//  1. ResolveDeploymentSpecEnv() → ConfigMapData + SecretData
//  2. Injects managed credentials (ANTHROPIC_API_KEY) into SecretData
//
// The deploy handler (handlers/deploy.go) does the same resolution and passes
// the result to SaveNormalizedSpec, which stores the key sets. They must match.
func TestResolvedKeys_MatchesSpecApplier(t *testing.T) {
	...
	const (
		agentName           = "key-test-agent"
		namespace           = "astro-keytest-0"
		buildID             = "b1"
		managedAnthropicKey = "sk-ant-managed-key"
	)

	// Spec with: 1 secret var with value, 2 optional secret vars (no value),
	// 1 plain var, plus interfaces (which adds GRPC_SERVER_ADDR to configmap).
	dsSpec := &spec.AstroDeploymentSpec{
		Spec:   "deployment/v1",
		Source: spec.DeploymentSource{Name: agentName, Build: buildID, Registry: "r.io"},
		Agent: spec.DeploymentAgent{
			Image: "r.io/key-test:latest", Replicas: 1,
			Resources: spec.DeploymentResources{CPU: "100m", Memory: "256Mi", CPULimit: "1", MemoryLimit: "1Gi"},
			Endpoints: map[string]spec.Endpoint{
				"http": {Port: 8080, Protocol: "http"},
			},
		...
		Interfaces: &spec.DeploymentInterfaces{
			Adapters:  []string{"web"},
			Image:     "messaging:latest",
			Resources: spec.DeploymentResources{CPU: "100m", Memory: "128Mi", CPULimit: "500m", MemoryLimit: "512Mi"},
			Endpoints: map[string]spec.Endpoint{
				"http": {Port: 8080, Protocol: "http"},
		...
		Variables: map[string]spec.Variable{
			"OPENAI_API_KEY":  {Value: "sk-real-key", Secret: true, Targets: []string{"agent"}},
			"SLACK_BOT_TOKEN": {Value: "", Secret: true, Targets: []string{"agent"}, Optional: true}, // optional, no value
		},
		Observability: spec.DeploymentObservability{Enabled: true, Port: 4318},
	}

	// --- Simulate what the deploy handler does ---
	...
	resolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)

	// Inject managed credentials (like deploy handler and spec applier both do)
	resolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey

	// Save to DB
	depID := deployid.New()
	_, err := store.SaveDeploymentPending(ds.SaveDeploymentParams{
		ID: depID, AccountID: accountID, AgentName: agentName,
		DisplayName: "Resolved Keys Test", BuildID: buildID, Namespace: namespace,
		SpecJSON: `{}`,
	}, func(tx *sql.Tx, id string) error {
		return ds.SaveNormalizedSpec(tx, id, dsSpec, resolved, nil, &ds.NormalizedSpecConfig{
			Namespace: namespace,
		})
	})
	if err != nil {
	...
	// --- Simulate what the spec applier does ---
	applierResolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)
	applierResolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey

	// --- Read back from DB and compare ---
	rk, err := store.GetResolvedKeys(depID)
	if err != nil {
		t.Fatalf("GetResolvedKeys: %v", err)
	}
	if rk == nil {
		t.Fatal("GetResolvedKeys returned nil — no resolved keys stored")
	}

	// Compare ConfigMap keys
	dbCMKeys := toSet(rk.ConfigMapKeys)
	applierCMKeys := toSet(mapKeys(applierResolved.ConfigMapData))

	for k := range applierCMKeys {
		if !dbCMKeys[k] {
			t.Errorf("ConfigMap key %q: present in spec applier output but missing from DB", k)
		}
	}
	for k := range dbCMKeys {
		if !applierCMKeys[k] {
			t.Errorf("ConfigMap key %q: present in DB but missing from spec applier output", k)
		}
	}

	// Compare Secret keys
	dbSecKeys := toSet(rk.SecretKeys)
	// Spec applier only includes secrets with actual values (HasSecretValues filters empties)
	applierSecKeys := make(map[string]bool)
	for k, v := range applierResolved.SecretData {
		if v != "" {
			applierSecKeys[k] = true
		}
	}

	for k := range applierSecKeys {
		if !dbSecKeys[k] {
			t.Errorf("Secret key %q: present in spec applier output but missing from DB", k)
		}
	}
	for k := range dbSecKeys {
		if !applierSecKeys[k] {
			t.Errorf("Secret key %q: present in DB but missing from spec applier output (drift false positive)", k)
		}
	}
}

// TestResolvedKeys_SasbotMatchesSpecApplier runs the same check against the
// real sasbot spec which has managed credentials + optional vars + multiple
// component types.
func TestResolvedKeys_SasbotMatchesSpecApplier(t *testing.T) {
	...
	const (
		namespace           = "astro-sasbot-rk-0"
		managedAnthropicKey = "sk-ant-managed-key"
	)

	dsSpec := parseSasbotSpec(t)

	// Fill in secret values (the spec has them empty)
	for k, v := range dsSpec.Variables {
		if v.Secret && v.Value == "" {
			v.Value = "test-" + k
			dsSpec.Variables[k] = v
		}
	}

	rctx := deployment.ResolveContext{
		Namespace:  namespace,
		AgentName:  dsSpec.Source.Name,
		BuildID:    dsSpec.Source.Build,
		SecretName: deployment.GenerateSecretName(dsSpec.Source.Name, dsSpec.Source.Build),
	}

	// Deploy handler path: resolve + inject managed + save
	resolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)
	resolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey

	depID := deployid.New()
	_, err := store.SaveDeploymentPending(ds.SaveDeploymentParams{
		ID: depID, AccountID: accountID, AgentName: dsSpec.Source.Name,
		DisplayName: "Sasbot RK Test", BuildID: dsSpec.Source.Build, Namespace: namespace,
		SpecJSON: sasbotSpecJSON,
	}, func(tx *sql.Tx, id string) error {
		return ds.SaveNormalizedSpec(tx, id, dsSpec, resolved, nil, &ds.NormalizedSpecConfig{
			Namespace: namespace,
	...
	// Spec applier path: same resolution
	applierResolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)
	applierResolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey
	...
	if rk == nil {
		t.Fatal("no resolved keys stored")
	}

	// ConfigMap keys
	dbCMKeys := toSet(rk.ConfigMapKeys)
	for k := range applierResolved.ConfigMapData {
		if !dbCMKeys[k] {
			t.Errorf("ConfigMap key %q: in spec applier but not in DB", k)
		}
	}
	for _, k := range rk.ConfigMapKeys {
		if _, ok := applierResolved.ConfigMapData[k]; !ok {
			t.Errorf("ConfigMap key %q: in DB but not in spec applier (false positive)", k)
		}
	}

	// Secret keys (only non-empty values)
	dbSecKeys := toSet(rk.SecretKeys)
	for k, v := range applierResolved.SecretData {
		if v != "" && !dbSecKeys[k] {
			t.Errorf("Secret key %q: in spec applier but not in DB", k)
		}
	}
	for _, k := range rk.SecretKeys {
		v := applierResolved.SecretData[k]
		if v == "" {
			t.Errorf("Secret key %q: in DB but has empty value in spec applier (false positive)", k)
		}
	}
}

// TestResolvedKeys_AfterRepair_StillMatches proves that RepairNormalizedSpec
// preserves the same resolved keys that the spec applier would produce.
//
// Currently FAILS because:
//   - Managed credentials (ANTHROPIC_API_KEY) are not re-injected during repair
//   - Optional secrets with no value are added to SecretData during repair but
//     the spec applier omits them
func TestResolvedKeys_AfterRepair_StillMatches(t *testing.T) {
	...
	const (
		agentName           = "repair-key-agent"
		namespace           = "astro-repairkey-0"
		buildID             = "b1"
		managedAnthropicKey = "sk-ant-managed-key"
	)
	...
	dsSpec := &spec.AstroDeploymentSpec{
		...
		Source: spec.DeploymentSource{Name: agentName, Build: buildID, Registry: "r.io"},
		Agent: spec.DeploymentAgent{
			Image: "r.io/repair-key:latest", Replicas: 1,
			Resources: spec.DeploymentResources{CPU: "100m", Memory: "256Mi", CPULimit: "1", MemoryLimit: "1Gi"},
			Endpoints: map[string]spec.Endpoint{
				"http": {Port: 8080, Protocol: "http"},
			},
		...
		Variables: map[string]spec.Variable{
			"OPENAI_API_KEY":  {Value: "sk-real-key", Secret: true, Targets: []string{"agent"}},
			"SLACK_BOT_TOKEN": {Value: "", Secret: true, Targets: []string{"agent"}, Optional: true},
		},
		Observability: spec.DeploymentObservability{Enabled: true, Port: 4318},
	}

	// --- Initial deploy (correct path) ---
	rctx := deployment.ResolveContext{
		Namespace: namespace, AgentName: agentName, BuildID: buildID,
		SecretName: deployment.GenerateSecretName(agentName, buildID),
	}
	resolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)
	resolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey

	specJSON, _ := json.Marshal(dsSpec)
	depID := deployid.New()
	_, err := store.SaveDeploymentPending(ds.SaveDeploymentParams{
		ID: depID, AccountID: accountID, AgentName: agentName,
		DisplayName: "Repair Keys Test", BuildID: buildID, Namespace: namespace,
	...
	rkBefore, err := store.GetResolvedKeys(depID)
	if err != nil || rkBefore == nil {
		t.Fatalf("GetResolvedKeys before repair: %v", err)
	}
	beforeSecKeys := toSet(rkBefore.SecretKeys)
	if !beforeSecKeys["ANTHROPIC_API_KEY"] {
		t.Fatal("ANTHROPIC_API_KEY missing from initial resolved keys — test setup is wrong")
	}

	// --- Run RepairNormalizedSpec (simulates admin repair or reconcile backfill) ---
	// Pass liveSecretData that mirrors what K8s would have (ANTHROPIC_API_KEY + OPENAI_API_KEY)
	liveSecretData := map[string][]byte{
		"OPENAI_API_KEY":    []byte("sk-real-key"),
		"ANTHROPIC_API_KEY": []byte(managedAnthropicKey),
	}
	_, _, _, err = store.RepairNormalizedSpec(depID, &ds.NormalizedSpecConfig{
		Namespace: namespace,
	}, liveSecretData)
	if err != nil {
	...
	// --- Read back and compare against spec applier output ---
	rkAfter, err := store.GetResolvedKeys(depID)
	if err != nil || rkAfter == nil {
		t.Fatalf("GetResolvedKeys after repair: %v", err)
	}

	// What the spec applier would produce
	applierResolved := deployment.ResolveDeploymentSpecEnv(dsSpec, rctx)
	applierResolved.SecretData["ANTHROPIC_API_KEY"] = managedAnthropicKey

	// Compare secret keys
	afterSecKeys := toSet(rkAfter.SecretKeys)
	applierSecKeys := make(map[string]bool)
	for k, v := range applierResolved.SecretData {
		if v != "" {
			applierSecKeys[k] = true
	...
	for k := range applierSecKeys {
		if !afterSecKeys[k] {
			t.Errorf("Secret key %q: in spec applier but missing from DB after repair", k)
		}
	}
	for k := range afterSecKeys {
		if !applierSecKeys[k] {
			t.Errorf("Secret key %q: in DB after repair but not in spec applier (false positive)", k)
		}
	}
}
