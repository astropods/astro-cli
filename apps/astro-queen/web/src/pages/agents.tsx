    ...
    <div>
      ...
      {data && (
        <div className="flex gap-4">
          <div className="flex-1 overflow-x-auto rounded-md border border-zinc-800">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/50">
                  <th className="px-4 py-2 text-left font-medium text-zinc-400">Account</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-400">Name</th>
                  <th className="px-4 py-2 text-right font-medium text-zinc-400">Builds</th>
                  <th className="px-4 py-2 text-right font-medium text-zinc-400">Published</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-400">Latest Build</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-400">Updated</th>
                  <th className="w-8"></th>
                </tr>
              </thead>
              <tbody>
                {data.agents?.map((a) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer border-b border-zinc-800/50 ${isSelected ? "bg-zinc-800/50" : "hover:bg-zinc-900/30"}`}
                      onClick={() => setSelected({ account: a.account_name, name: a.name })}
                    >
                      <td className="px-4 py-2 text-zinc-400">{a.account_name}</td>
                      ...
                      <td className="px-4 py-2 font-mono text-xs text-zinc-500">{truncateUUID(a.latest_build_id)}</td>
                      <td className="px-4 py-2 text-zinc-500">{formatDateTime(a.updated_at)}</td>
                      <td className="px-2">
...
function BuildsPanel({ account, name, onClose }: { account: string; name: string; onClose: () => void }) {
  ...
  return (
    <div className="w-80 shrink-0 rounded-md border border-zinc-800 bg-zinc-900/30 p-3">
      ...
      {data?.builds?.map((b) => (
        <div key={b.build_id} className="mb-2 rounded border border-zinc-800 px-2.5 py-1.5 text-xs">
          <p className="font-mono text-amber">{truncateUUID(b.build_id)}</p>
          {b.tagged_version && <p className="text-zinc-400">v{b.tagged_version}</p>}
