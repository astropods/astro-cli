...
- **integrations** — Services needed to run the agent. Credentials from `.env`.
- **interfaces** — How users talk to the agent: `web` (local UI), Slack, HTTP APIs. Config from `.env`.
- **ingestion** (optional) — Data pipelines with cron, manual, or startup triggers.

Edit this file to add/remove integrations, interfaces, or knowledge, and ensure `.env` has the matching keys.
