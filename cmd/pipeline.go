...
func (p *PushPipeline) Build() *PushPipeline {
	return p.step(func() error {
		...
		for _, comp := range p.components {
			...
			fmt.Fprintf(progressOut, "%s→%s Building %s[%s %s]%s %s%s%s",
...
func (p *PushPipeline) retagLocal() error {
	if p.cfg.SkipBuild {
		...
		fmt.Fprintf(progressOut, "%s→%s Skipping image push %s(local dev server detected)%s\n", colorCyan, colorReset, colorDim, colorReset)
		return nil
	}
...
func (p *PushPipeline) UploadReadmeAssets() *PushPipeline {
	return p.step(func() error {
		...
		if err != nil {
			printStepFail()
			fmt.Fprintf(progressOut, "  %s!%s could not upload images, continuing with relative links: %v\n", colorYellow, colorReset, err)
			return nil
		}
...
func (p *PushPipeline) printSkippedComponents() {
	if p.astroSpec.Agent.Build == nil && p.astroSpec.Agent.Image != "" {
		fmt.Fprintf(progressOut, "%s→%s Skipping %s[agent]%s using image: %s%s%s\n",
	...
	for name, model := range p.astroSpec.Models {
		...
		if model.Container == nil || model.Container.Build == nil {
			if resolved.Image != "" {
				fmt.Fprintf(progressOut, "%s→%s Skipping %s[model: %s]%s using image: %s%s%s\n",
	...
	for name, knowledge := range p.astroSpec.Knowledge {
		...
		if container.Build == nil && container.Image != "" {
			fmt.Fprintf(progressOut, "%s→%s Skipping %s[knowledge: %s]%s using image: %s%s%s\n",
	...
	for name, tool := range p.astroSpec.Integrations {
		if tool.Container == nil || tool.Container.Build == nil {
			if tool.Container != nil && tool.Container.Image != "" {
				fmt.Fprintf(progressOut, "%s→%s Skipping %s[integration: %s]%s using image: %s%s%s\n",
	...
	for name, ingestion := range p.astroSpec.Ingestion {
		if ingestion.Container.Build == nil && ingestion.Container.Image != "" {
			fmt.Fprintf(progressOut, "%s→%s Skipping %s[ingestion: %s]%s using image: %s%s%s\n",
...
func (p *PushPipeline) PrintSuccess() {
	...
	fmt.Fprintln(progressOut)
	fmt.Fprintln(progressOut, theme.Box(lines))
	if urlBelowBox != "" {
		fmt.Fprintln(progressOut, urlBelowBox)
	}
	fmt.Fprintln(progressOut)
}
