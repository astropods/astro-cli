...
const SLACK_ICON = `<path d="M5.042 15.165a2.528 2.528 0 0 1-2.52 2.523A2.528 2.528 0 0 1 0 15.165a2.527 2.527 0 0 1 2.522-2.52h2.52v2.52zm1.271 0a2.527 2.527 0 0 1 2.521-2.52 2.527 2.527 0 0 1 2.521 2.52v6.313A2.528 2.528 0 0 1 8.834 24a2.528 2.528 0 0 1-2.521-2.522v-6.313zM8.834 5.042a2.528 2.528 0 0 1-2.521-2.52A2.528 2.528 0 0 1 8.834 0a2.528 2.528 0 0 1 2.521 2.522v2.52H8.834zm0 1.271a2.528 2.528 0 0 1 2.521 2.521 2.528 2.528 0 0 1-2.521 2.521H2.522A2.528 2.528 0 0 1 0 8.834a2.528 2.528 0 0 1 2.522-2.521h6.312zM18.956 8.834a2.528 2.528 0 0 1 2.522-2.521A2.528 2.528 0 0 1 24 8.834a2.528 2.528 0 0 1-2.522 2.521h-2.522V8.834zm-1.27 0a2.528 2.528 0 0 1-2.523 2.521 2.527 2.527 0 0 1-2.52-2.521V2.522A2.527 2.527 0 0 1 15.163 0a2.528 2.528 0 0 1 2.523 2.522v6.312zM15.163 18.956a2.528 2.528 0 0 1 2.523 2.522A2.528 2.528 0 0 1 15.163 24a2.527 2.527 0 0 1-2.52-2.522v-2.522h2.52zm0-1.27a2.527 2.527 0 0 1-2.52-2.523 2.527 2.527 0 0 1 2.52-2.52h6.315A2.528 2.528 0 0 1 24 15.163a2.528 2.528 0 0 1-2.522 2.523h-6.315z" fill="currentColor"/>`;
const GITHUB_ICON = `<path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z" fill="currentColor"/>`;
const LINEAR_ICON = `<path d="M1.16 17.37l5.93 5.93a11.86 11.86 0 0 1-5.93-5.93zM.5 13.5l10 10a11.93 11.93 0 0 0 3.86-1.25L1.75 9.64A11.93 11.93 0 0 0 .5 13.5zm2.1-6.07l13.97 13.97a11.8 11.8 0 0 0 2.56-2.56L5.16 4.87a11.8 11.8 0 0 0-2.56 2.56zM9.64 1.75L22.25 14.36a11.93 11.93 0 0 0 1.25-3.86l-10-10a11.93 11.93 0 0 0-3.86 1.25zM17.37 1.16l5.47 5.47A12.01 12.01 0 0 0 17.37 1.16z" fill="currentColor"/>`;

const defaultData: Omit<CardData, "avatar" | "colors"> = {
  ...
  integrations: [
    ...
    { name: "Linear", icon: LINEAR_ICON },
  ],
  barcodeId: "AGT-7f3a9b2e-01",
  qrUrl: "https://astro.postman.com/agents/research-assistant",
};
...
const samples: CardSample[] = [
  ...
  identityAvatar("scenario/no-integrations", "No integrations", {
    displayName: "Bare Agent",
    integrations: [],
  }),
  identityAvatar("scenario/no-stats", "No stats", {
  ...
  identityAvatar("scenario/long-handle", "Long handle", {
    displayName: "Handle Test",
    stats: [
      { label: "Deployed by", account: { fullName: "Christopher Wellington-Smythe", handle: "christopher.wellington-smythe", avatarUrl: null } },
      { label: "Version", value: "2.0.0" },
    ],
  }),
  identityAvatar("scenario/very-long-handle", "Truncated handle", {
    displayName: "Truncation Test",
    stats: [
      { label: "Deployed by", account: { fullName: "Bartholomew Fitzwilliam Harrington III", handle: "bartholomew.fitzwilliam-harrington-the-third", avatarUrl: null } },
...
function setupDownloads() {
  document.getElementById("dl-svg")?.addEventListener("click", async () => {
    if (lastSvg) await downloadSvg(lastSvg, { name: defaultData.name, id: defaultData.barcodeId ?? "unknown" });
  });
  document.getElementById("dl-png")?.addEventListener("click", async () => {
    if (lastSvg) await downloadPng(lastSvg, { name: defaultData.name, id: defaultData.barcodeId ?? "unknown" });
  });
}
